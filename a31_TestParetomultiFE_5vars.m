...
% -- 5 points
% M0 = readmatrix("jCOER multi test 01.xlsx"); % GDL CL j0 alpha phi
...
% -- 3 points
M0 = readmatrix("jCOER multi test 03.xlsx"); % GDL CL j0 alpha phi
...
lb = [min(xGDL);min(xCL);min(j0);min(alphac);-1.8];
ub = [max(xGDL);max(xCL);max(j0);max(alphac);-1.2];
% options = optimoptions('paretosearch','ParetoSetSize',100,...
%     'PlotFcn',{'psplotparetof' 'psplotparetox'});
options = optimoptions('paretosearch','ParetoSetSize',500);

% s.t. GDL > CL
A   = [-1,1,0,0,0];
b   = 0;

% fix one Catalyst mA/cm^2 
Aeq = [0,0,1,0,0;...
    0,0,0,1,0];
beq = [2.71e-3;0.44];
[xx,fval,exitflag,output,residuals] = paretosearch(@HHtarget,5,A,b,Aeq,beq,lb,ub,@HHcirclecons,options);
% xx = paretosearch(@HHtarget,3,[],[],[],[],lb,ub,@HHcirclecons)

% remove the data s.t. GDL < CL
...
hold on
plot(X(:,end),MFE(:,end),'.');
plot(xx(:,end),mean(kfoldfun(TreeModelCV,f))'+predict(GaussModel,xx(:,end)),'r*');
...
e0 = 1.6021766e-19;
Prate = 2./28;
Itotal = Prate.*2.*e0.*96485./FE;
PowerConsumed = Itotal.*abs(x(end));
Eelectrolyser = PowerConsumed./2;
y(2) = Eelectrolyser;
%%%%%%
% y(2) = -x(3)./FE;
y(3) = -jCOER;
